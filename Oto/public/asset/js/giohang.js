...
function addItemToCart(productId, title, price, image) {
  ...
  var cartRowContents = `
    ...
    <span class="cart-price cart-column" style="display:contents;">${price.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</span>
...
function updateCartTotal() {
  ...
  // Sử dụng .toLocaleString() để định dạng lại số tiền
  var totalFormatted = total.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' });

  document.getElementsByClassName('cart-total-price')[0].innerText = totalFormatted;
}
